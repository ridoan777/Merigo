...
class LargeMealGenerationAgent implements Agent, HasStructuredOutput, HasTools
{
    use Promptable;
    public string $model;

    public ?int $startingDay = null;
    public ?int $endingDay = null;
    public ?int $duration;
    public ?array $oldMeals = null;
    ...
    public function __construct(
        ...
        public ?array $generatedMeals
    ) {
        $this->duration = $this?->TARGET_DURATION ? $this?->TARGET_DURATION : ($this->TARGET_PREFERENCE?->duration ?? 3);
        Log::info('Large Agent constructor is hit. Meal duration=', [$this->duration]);

        if ($this->generatedMeals) {
            $this->oldMeals = $this->generatedMeals ?? null;
        }

        $this->startingDay = $this->START_DAY ?? 1;
        $this->endingDay = $this->END_DAY ?? $this->duration;
    }
    ...
    {
        Log::info('Agent instructions is hit', [$this->user->id]);
    ...
    {
        Log::info('Agent tools is hit', [$this->user->id]);
        return [
    ...
    public function schema(JsonSchema $schema): array
    {
        ...
        for ($i = $this->startingDay; $i <= $this->endingDay; $i++) {
    ...
    protected function buildPrompt(): string
    {
        ...
        $splitMessage = null;
        if ($this->oldMeals) {
            $existingMeals = json_encode($this->oldMeals, JSON_UNESCAPED_UNICODE);

            $splitMessage = "- As this is a chunked meal generation, these food titles were already generated by the LLM. {$existingMeals}. Try to generate something new but relevant to the context.";
        }
        // ------------------ CHUNK CALL MESSAGE ------------------

        $dayRange = $this->TARGET_DURATION ? ($this->endingDay - $this->startingDay + 1) : $this->duration;
        ...
        return "
            ...
            Your task:
            Generate a personalized meal plan from day-{$this->startingDay} to day-{$this->endingDay} based on the user's stored preferences.
            ...
            - The planning object MUST contain EXACTLY {$dayRange} days
            - Generate ONLY days from day-{$this->startingDay} to day-{$this->endingDay}
            - Do NOT generate any other days
            - If planning contains fewer than {$dayRange} days, REGENERATE internally before responding
            - DO NOT return partial results
            - You must explicitly generate ALL days from day-{$this->startingDay} to day-{$this->endingDay} for {$dayRange} days
            ...
            - Repeat the same structure for ALL days from day-{$this->startingDay} to day-{$this->endingDay}
