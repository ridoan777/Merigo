<x-app-layout>
	...
	@php
		$breadCrumbs = [
			['label' => 'Chat Blocklists', 'url' => 'backend_chat_blocklist_index', 'icon' => 'noMessage'],
		];
	 @endphp
	...
	<!-- Start Main Widgets -->
	<x-indicators.page_header_widget pageName="Chatting: All Blocked Messenger Records" />
	<!-- End Main Widgets -->
	...
	<div class="hs-accordion-group">
		...
		<div id="hs-destroy-heading-two"
			...
			<x-ui_items.accordion.accordion_toggle_button toggleLabel="Chat Blocking History" />
	...
	<script>
		$(function () {
			window.dataTableDisplay = $('#dataTableDisplay').DataTable({
				...
				autoWidth: false,
				ajax: {
					url: '{{ route('backend_chat_blocklist_datatable') }}',
					type: 'POST',
				...
				columns: [
					...
					{ data: 'blocked', className: 'px-3 py-2 text-left', orderable: false },
					{ data: 'composites', className: 'px-3 py-2 text-center', orderable: false },
					{ data: 'phase', className: 'px-3 py-2 text-center', orderable: false },
					{ data: 'reason', className: 'px-3 py-2 text-left' },
					{ data: 'status_label', className: 'px-3 py-2 text-center', orderable: false },
