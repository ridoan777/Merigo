<x-app-layout>
    ...
    <!-- Start Main Widgets -->
    <x-indicators.page_header_widget pageName="Merchandise List" />
    <!-- End Main Widgets -->
    ...
    <!-- -------------------------------- APP AREA Starts -------------------------------- -->
    @include('Admin.sidebar.Merchandises.create')
    <div class="px-6">

        <div class="">
            <form action="{{ route('backend_merchandise_bulk_delete') }}" method="POST">
                @csrf
                ...
                <table id="dataTableDisplay"
                    ...
                    <thead class="text-xs text-gray-400 uppercase bg-gray-100 rounded-t-xl">
                        <tr>
                            ...
                            <th class="min-w-30">Name</th>
                            <th class="min-w-40">Image</th>
                            <th class="min-w-24">Points Need</th>
                            <th>Description</th>
                            <th class="min-w-40">Creator</th>
                            <th>Status</th>
                            <th>Created At</th>
                            <th>Action</th>
                        </tr>
    ...
    <script>
        $(function () {
            window.dataTableDisplay = $('#dataTableDisplay').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{{ route("backend_merchandise_datatable") }}',
                    type: 'POST',
                    headers: {
                ...
                columns: [
                    ...
                    { data: 'merc_uid', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'name', className: 'px-3 py-2 text-left', orderable: true, searchable: true },
                    { data: 'image_block', className: 'px-3 py-2 text-center', orderable: false, searchable: false },
                    { data: 'point_block', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'description', className: 'px-3 py-2 text-left', orderable: true, searchable: true },
                    { data: 'creator_block', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
