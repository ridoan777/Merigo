<x-app-layout>
    ...
    @php
        $breadCrumbs = [
            ['label' => 'Bar Wallets', 'url' => 'backend_bar_wallet_index', 'icon' => 'barShop'],
            ['label' => 'Merigo Wallets', 'url' => 'backend_merigo_wallet_index', 'icon' => 'cubic'],
            ['label' => 'Transactions'],
        ];
    @endphp
    ...
    <!-- Start Main Widgets -->
    <x-indicators.page_header_widget pageName="All Wallet Transactions" />
    <!-- End Main Widgets -->
    ...
    <div class="px-6">
        ...
        <div class="">
            <form action="{{ route('backend_wallet_trx_bulk_delete') }}" method="POST">
                @csrf
                ...
                <table id="dataTableDisplay"
                    ...
                    <thead class="text-xs text-gray-400 uppercase bg-gray-100 rounded-t-xl">
                        <tr>
                            ...
                            <th>SN</th>
                            <th>DB ID</th>
                            <th class="max-w-32 text-center">TRX UID</th>
                            <th class="min-w-40">User</th>
                            <th class="min-w-32">Wallet</th>
                            <th>Type</th>
                            <th>Amount</th>
                            <th>Phase</th>
                            <th>Note</th>
    ...
    <script>
        $(function () {
            window.dataTableDisplay = $('#dataTableDisplay').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{{ route("backend_wallet_trx_datatable") }}',
                    type: 'POST',
                    headers: {
                ...
                columns: [
                    ...
                    { data: 'uid', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'user_block', className: 'px-3 py-2 text-center', orderable: false, searchable: true },
                    { data: 'wallet_block', className: 'px-3 py-2 text-center', orderable: false, searchable: true },
                    { data: 'transaction_type_label', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'amount_label', className: 'px-3 py-2 text-center', orderable: true, searchable: false },
                    { data: 'phase_label', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    ...
                    { data: 'created_at_formatted', className: 'px-3 py-2 text-center', orderable: true, searchable: false },
