<x-app-layout>
    ...
    @php
        $breadCrumbs = [
            ...
            ['label' => 'Transactions', 'url' => 'backend_wallet_trx_index', 'icon' => 'dollarSackLine'],
        ];
    @endphp
    ...
    <!-- Start Main Widgets -->
    <x-indicators.page_header_widget pageName="Bar Wallets" />
    <!-- End Main Widgets -->
    ...
    <div class="px-6">
        ...
        <div class="">
            <form action="{{ route('backend_bar_wallet_bulk_delete') }}" method="POST">
                @csrf
                ...
                <table id="dataTableDisplay"
                    ...
                    <thead class="text-xs text-gray-400 uppercase bg-gray-100 rounded-t-xl">
                        <tr>
                            ...
                            <th class="min-w-40">Bar</th>
                            <th class="min-w-32">Wallet</th>
                            <th class="min-w-24">Last Trx</th>
                            <th>Status</th>
                            <th>Created At</th>
                            <th>Action</th>
                        </tr>
    ...
    <script>
        $(function () {
            window.dataTableDisplay = $('#dataTableDisplay').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{{ route("backend_bar_wallet_datatable") }}',
                    type: 'POST',
                    headers: {
                ...
                columns: [
                    ...
                    { data: 'bar_wallet_uid', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'user_block', className: 'px-3 py-2 text-center', orderable: false, searchable: true },
                    { data: 'bar_block', className: 'px-3 py-2 text-center', orderable: false, searchable: true },
                    { data: 'wallet_block', className: 'px-3 py-2 text-center', orderable: false, searchable: false },
                    { data: 'last_transaction', className: 'px-3 py-2 text-center', orderable: false, searchable: false },
