<x-app-layout>
    ...
    @php
        $breadCrumbs = [
            ['label' => 'Create Event', 'url' => 'backend_event_create', 'icon' => 'ribbon'],
            ['label' => 'Bar List'],
        ];
    @endphp
    ...
    <!-- Start Main Widgets -->
    <x-indicators.page_header_widget pageName="Event List" />
    <!-- End Main Widgets -->
    ...
    <!-- -------------------------------- APP AREA Starts -------------------------------- -->
    @include('Admin.sidebar.Referrals.rules')
    <div class="px-6">

        <div class="">
            <form action="{{ route('backend_events_bulk_delete') }}" method="POST">
                @csrf
                ...
                <table id="dataTableDisplay"
                    ...
                    <thead class="text-xs text-gray-400 uppercase bg-gray-100 rounded-t-xl">
                        <tr>
                            ...
                            <th class="min-w-30">Event</th>
                            <th class="min-w-40">Bar Details</th>
                            <th class="min-w-40">Galleries</th>
                            <th class="min-w-24">Schedule</th>
                            <th>Points Giveaway</th>
                            <th class="min-w-40">Event Creator</th>
                            <th>Status</th>
                            <th>Created At</th>
                            <th>Action</th>
                        </tr>
    ...
    <script>
        $(function () {
            window.dataTableDisplay = $('#dataTableDisplay').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{{ route("backend_event_datatable") }}',
                    type: 'POST',
                    headers: {
                ...
                columns: [
                    ...
                    { data: 'SN', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'id', className: 'px-3 py-2 text-center' },
                    { data: 'event_uid', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'event_details', className: 'px-3 py-2 text-left', orderable: true, searchable: true },
                    { data: 'bar_details', className: 'px-3 py-2 text-left', orderable: true, searchable: true },
                    { data: 'galleries', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'schedule', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'points', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
                    { data: 'creator', className: 'px-3 py-2 text-center', orderable: true, searchable: true },
